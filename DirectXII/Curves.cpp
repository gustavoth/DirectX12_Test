...
void Curves::Update()
{
    ...
    else if (input->KeyPress(VK_RETURN)) {
        Subdivision();
    }
}
...
void Curves::Draw()
{
    ...
    graphics->CommandList()->IASetVertexBuffers(0, 1, line->VertexBufferView());
    graphics->CommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_LINESTRIP);

    // submete comandos de desenho
    graphics->CommandList()->DrawInstanced(index, 1, 0, 0);
...
void Curves::BuildLine()
{
    // cria malha 3D
    line = new Mesh("Curve");

    // ajusta atributos da malha 3D
    line->vertexByteStride = sizeof(Vertex);
    line->vertexBufferSize = maxVertives * sizeof(Vertex);

    // aloca recursos para o vertex buffer
    graphics->Allocate(line->vertexBufferSize, &line->vertexBufferCPU);
    graphics->Allocate(UPLOAD, line->vertexBufferSize, &line->vertexBufferUpload);
    graphics->Allocate(GPU, line->vertexBufferSize, &line->vertexBufferGPU);
}

void Curves::setPoint(float x, float y)
{
    vertices[index] = { XMFLOAT3(x, y, 0), XMFLOAT4(Colors::White) };
    index++;



    // copia vértices para o armazenamento local da malha
    graphics->Copy(vertices, line->vertexBufferSize, line->vertexBufferCPU);

    // copia vértices para o buffer da GPU usando o buffer de Upload
    graphics->ResetCommands();
    graphics->Copy(vertices, line->vertexBufferSize, line->vertexBufferUpload, line->vertexBufferGPU);
    graphics->SubmitCommands();
}

void Curves::resetPoints()
{
    for (int i = 0; i < index; i++)
    {
        vertices[i] = { XMFLOAT3(0, 0, 0), XMFLOAT4(Colors::White) };
    }

    index = 0;
}

void Curves::Subdivision()
{
    if (index * 2 - 2 < maxVertives) {
        Vertex v[maxVertives];
        int count = 0;

        for (int i = 0; i < index - 1; i++)
        {
            float x = 3.f / 4.f * vertices[i].Pos.x + 1.f / 4.f * vertices[i + 1].Pos.x;
            float y = 3.f / 4.f * vertices[i].Pos.y + 1.f / 4.f * vertices[i + 1].Pos.y;
            v[count] = { XMFLOAT3(x, y, 0), XMFLOAT4(Colors::White) };
            count++;

            x = 1.f / 4.f * vertices[i].Pos.x + 3.f / 4.f * vertices[i + 1].Pos.x;
            y = 1.f / 4.f * vertices[i].Pos.y + 3.f / 4.f * vertices[i + 1].Pos.y;
            v[count] = { XMFLOAT3(x, y, 0), XMFLOAT4(Colors::White) };
            count++;
        }
